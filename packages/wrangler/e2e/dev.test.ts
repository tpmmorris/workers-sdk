...
`,
	...
	it("should expose captured emails through the local explorer API", async ({
		...
		expect(await sentResponse.json()).toEqual({
			messageId: "<e2e-sent@example.com>",
		});
		...
		const sentItem = sentList.result.find(
			(email) => email.messageId === "<e2e-sent@example.com>"
		);
		...
		const sentDetail = await fetchJson<{
			...
			`${apiUrl}/email/sending/${encodeURIComponent("<e2e-sent@example.com>")}`
		);

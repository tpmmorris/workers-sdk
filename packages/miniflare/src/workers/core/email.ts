...
export async function handleEmail(
	...
	// Capture this email for the local explorer "Routing" interface.
	const rawBase64 = bytesToBase64(incomingEmailRaw);
	...
	async function storeReceivedEmail(): Promise<void> {
		...
		try {
			...
			if (store !== undefined) {
				...
				if (rawBase64.length > 64 * 1024) {
					const metadata: StoredRoutingEmailMetadata = emailMetadata;

 ...
 *
 * When a `worker` is selected resolve a direct service binding to it and
 * invoke `handleEmail`. This avoids routing the delivery back
 * through the entry worker.
 *
 * When no worker is selected deliver via the entry worker's public URL.
 *
...
export async function sendTestEmail(
	...
	// Derive the Message-ID exactly as the `send_email` binding does.
	const messageId =
